...
- Колонку с датой (называется dt)
- Колонку с целевой переменной (цена, называется price)
...
expert_features:  # экспертный выбор (в случае, если нет аналитически отобранных)
  - 'Stock-to-Use'
  - 'Brent'
  - 'DXY'
...
- `results/*/ml_feature_importance_detailed.xlsx` - важность признаков (детально)
- `results/*/ml_feature_importance_pivot.xlsx` - важность признаков (сводная)
- `results/*/ml_feature_importance_aggregated.xlsx` - важность признаков (агрегированная)
- `results/*/ml_all_predictions.png` - график прогнозов
- `results/*/ml_metrics_comparison.png` - сравнение метрик
- `results/*/ml_feature_importance_*.png` - графики важности признаков
...
| SF Oil    | `configs/sf_oil.yaml` | Цены на подсолнечное масло |
| Palm Oil    | `configs/palm_oil.yaml` | Цены на пальмовое масло |
...
Библиотека автоматически рассчитывает и сохраняет важность признаков для всех ML моделей:

### Методы расчёта
| Модель | Метод |
|--------|-------|
| CatBoost, XGBoost, LightGBM, RF, AdaBoost | `feature_importances_` (native) |
| Ridge, Lasso, SGD | `coef_` (coefficients) |
| KNN, SVR | SHAP values |

### Выходные файлы
- `ml_feature_importance_detailed.xlsx` — важность по каждой модели
- `ml_feature_importance_pivot.xlsx` — сводная таблица (признак × модель)
- `ml_feature_importance_aggregated.xlsx` — средняя важность по всем моделям

### Графики
- `ml_feature_importance_{model}.png` — Top-15 для лучшей модели
- `ml_feature_importance_heatmap.png` — Сравнение моделей (heatmap)
- `ml_feature_importance_aggregated.png` — Агрегированная важность
...
importance = ml.get_feature_importance('catboost', top_n=10)

# Все модели
all_importance = ml.get_all_feature_importances()

# Агрегированная важность
agg_importance = ml.get_aggregated_importance(method='mean')  # или 'median', 'rank'
```
